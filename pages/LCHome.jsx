...
function injectGA() {
  // COPPA: only inject GA if parent consent confirmed or 13+ learner
  const mode = localStorage.getItem("lc_age_mode") || "";
  const consent = localStorage.getItem("lc_consent_status") || "";
  if (mode !== "leader" && consent !== "approved" && consent !== "not_required") return;
...
export default function LCHome() {
    ...
    try {
      ...
      if (dbId) {
        ...
        await LearnerProfile.update(dbId, {
          xp: newXp, legacy_level: newLevel,
  ...
  return (
    <div style={{ fontFamily: "sans-serif", minHeight: "100vh", background: T.bg, color: T.text, paddingBottom: 90 }}>
      {/* Top bar */}
      <div style={{ background: "linear-gradient(135deg, #1a1e35, #0e1020)", padding: "14px 20px", borderBottom: `1px solid ${T.border}` }}>
        <div style={{ maxWidth: 680, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <img
              ...
              style={{ width: 38, height: 38, borderRadius: 10, objectFit: "cover", border: `2px solid ${T.gold}40` }}
              onError={e => e.target.style.display = "none"}
            />
            <div>
              <div style={{ fontSize: 10, color: T.muted, letterSpacing: 1 }}>{today}</div>
